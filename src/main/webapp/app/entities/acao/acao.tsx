    ...
    <div>
      ...
      <div className="table-responsive">
        {acaoList && acaoList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>ID</th>
                <th>Data Criação</th>
                <th>Usuário Criação Ação</th>
                <th>Pendente</th>
                <th>Data Execução Ação</th>
                <th>Ativa</th>
                <th>Foto</th>
                <th>Observações</th>
                <th>Cadastro Doação</th>
                <th>Solicitação</th>
              ...
              {acaoList.map((acao, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {acao.foto ? (
                      <div>
                        {acao.fotoContentType ? <a onClick={openFile(acao.fotoContentType, acao.foto)}>Abrir &nbsp;</a> : null}
                        <span>
                  ...
                  <td>{acao.solicitacao ? <Link to={`solicitacao/${acao.solicitacao.id}`}>{acao.solicitacao.solicitante}</Link> : ''}</td>
                  <td>{acao.cadastroUser ? <Link to={`cadastro-user/${acao.cadastroUser.id}`}>{acao.cadastroUser.nome}</Link> : ''}</td>
          ...
          !loading && <div className="alert alert-warning">Ações não encontradas</div>
        )}
      </div>
    </div>
  );
